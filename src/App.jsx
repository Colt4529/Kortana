  ...
  return (
    <div style={{ padding:"32px clamp(18px,5vw,48px) 0" }}>
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:"clamp(14px,3vh,20px)" }}>
        <span style={{ fontFamily:"monospace", fontSize:9, color:"#555" }}>&gt;_</span>
        <span style={{ fontFamily:"monospace", fontSize:10, fontWeight:600, letterSpacing:"2.5px", color:col, flexShrink:0 }}>[ {label.toUpperCase().replace(/ /g,"_")} ]</span>
        <div style={{ flex:1, height:"0.5px", background:`linear-gradient(to right,${col}45,transparent)` }} />
      </div>
      {children}
    </div>
  );
...
function Empty({ label }) {
  return (
    <div style={{ textAlign:"center", padding:"70px 0", fontFamily:"monospace" }}>
      <div style={{ fontSize:9, color:"#2a3a55", letterSpacing:"2px", marginBottom:6 }}>&gt;_ NO_ENTRIES_FOUND</div>
      <div style={{ fontSize:10, color:"#3a4a6a" }}>// {label}</div>
    </div>
  );
}
...
function ConnectPSNSheet({ onConnect, onClose }) {
  ...
  const bookmarklet = `javascript:(function(){var c=document.cookie.split(';').map(s=>s.trim()).find(s=>s.startsWith('npsso='));if(c){var v=c.split('=').slice(1).join('=');navigator.clipboard.writeText(v).then(function(){alert('\\u2713 Token copied! Go back to Kortana and paste it.');}).catch(function(){prompt('Copy this token:',v);});}else{alert('Not found. Sign in to PlayStation.com first.');}})()`;
  ...
  const handlePaste = e => {
    ...
    setNpsso(val); setError(""); setProfile(null);
    if (val.length > 20) setTimeout(() => verify(val), 100);
  };

  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.87)", zIndex:400, display:"flex", alignItems:"flex-end", justifyContent:"center", backdropFilter:"blur(14px)" }}>
      <div style={{ background:C.surface, borderRadius:"16px 16px 0 0", width:"100%", maxWidth:440, paddingBottom:40, border:`0.5px solid ${C.border}`, borderBottom:"none", animation:"slideUp .22s ease", overflow:"hidden", maxHeight:"90vh", overflowY:"auto" }}>
        <StripeBar height={3} />
        <div style={{ display:"flex", justifyContent:"center", padding:"14px 0 6px" }}>
          <div style={{ width:36, height:3, borderRadius:2, background:C.border }} />
        </div>
        <div style={{ padding:"4px 24px 0" }}>
          ...
          <div style={{ fontSize:12, color:"#666", marginBottom:18, lineHeight:1.6 }}>
            Link your PSN account to see your library, trophies, and monthly stats.
          </div>

          {/* Step 1: Open App */}
          <div style={{ display:"flex", gap:8, marginBottom:18 }}>
            <a href="https://www.playstation.com/en-us/" target="_blank" rel="noopener noreferrer"
          ...
          {/* Step 2: Bookmarklet */}
          <div style={{ background:C.faint, border:`0.5px solid ${C.border}`, borderRadius:10, padding:"14px 16px", marginBottom:18 }}>
            <div style={{ fontSize:11, color:"#444", letterSpacing:"1.5px", textTransform:"uppercase", marginBottom:8, fontWeight:600 }}>2. Add this to your bookmarks bar</div>
            <div style={{ fontSize:12, color:C.muted, marginBottom:10, lineHeight:1.6 }}>
              Drag the button below to your browser's bookmarks bar. Then visit PlayStation.com, click it — your token copies automatically.
            </div>
            <a href={bookmarklet} style={{ display:"inline-block", padding:"8px 16px", borderRadius:8, background:"#003791", color:"#fff", fontSize:12, fontWeight:700, textDecoration:"none", letterSpacing:"0.5px", cursor:"grab" }}>
              📋 Kortana Helper
            </a>
            <div style={{ fontSize:11, color:"#555", marginTop:8 }}>← Drag this to your bookmarks bar</div>
          </div>

          {/* Step 3: Paste */}
          <div style={{ fontSize:10, color:"#444", letterSpacing:"2px", textTransform:"uppercase", marginBottom:8, fontWeight:600 }}>3. Paste your token</div>
          <textarea
            value={npsso}
            onChange={e => { setNpsso(e.target.value); setError(""); setProfile(null); }}
            onPaste={handlePaste}
            placeholder="Paste token here after clicking the bookmarklet…"
            rows={3}
          ...
          {profile ? (
            <>
              <div style={{ display:"flex", alignItems:"center", gap:14, padding:"14px 16px", background:C.faint, borderRadius:10, marginBottom:16, border:`0.5px solid ${C.green}44` }}>
                {profile.avatarUrl && <img src={profile.avatarUrl} style={{ width:46, height:46, borderRadius:"50%", objectFit:"cover" }} alt="" />}
                <div>
                  <div style={{ fontSize:14, fontWeight:500 }}>{profile.onlineId}</div>
                  <div style={{ fontSize:11, color:C.green }}>PSN account verified ✓</div>
                </div>
              </div>
              <button onClick={() => onConnect(npsso.trim(), profile)} style={{ width:"100%", padding:14, borderRadius:10, border:"none", background:"#003791", color:"#fff", fontWeight:600, fontSize:14, cursor:"pointer", letterSpacing:"1px" }}>
                Connect PlayStation
              </button>
            </>
          ) : (
...
function ConnectXboxSheet({ onConnect, onClose }) {
  ...
  const verify = async () => {
    ...
    if (!pu) { setError("Invalid key. Make sure you copied it from openxbl.com."); setLoading(false); return; }
    ...
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.87)", zIndex:400, display:"flex", alignItems:"flex-end", justifyContent:"center", backdropFilter:"blur(14px)" }}>
      <div style={{ background:C.surface, borderRadius:"16px 16px 0 0", width:"100%", maxWidth:440, paddingBottom:40, border:`0.5px solid ${C.border}`, borderBottom:"none", animation:"slideUp .22s ease", overflow:"hidden" }}>
        ...
        <div style={{ padding:"4px 24px 0" }}>
          ...
          <div style={{ fontSize:12, color:"#666", marginBottom:18, lineHeight:1.6 }}>
            Link your Xbox account to see your library, achievements, and monthly stats.
          </div>

          <a href="https://www.xbox.com/en-US/" target="_blank" rel="noopener noreferrer"
            style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8, padding:"12px", borderRadius:10, marginBottom:18,
              background:"rgba(16,60,16,.35)", border:"0.5px solid rgba(16,124,16,.4)", color:C.green, fontWeight:600, fontSize:14,
              textDecoration:"none" }}>
            Open Xbox App ↗
          </a>

          <div style={{ background:C.faint, border:`0.5px solid ${C.border}`, borderRadius:10, padding:"14px 16px", marginBottom:16 }}>
            <div style={{ fontSize:11, color:"#444", letterSpacing:"1.5px", textTransform:"uppercase", marginBottom:8, fontWeight:600 }}>Get your free API key (30 sec)</div>
            <div style={{ fontSize:12, color:C.muted, lineHeight:1.9 }}>
              1. Go to <a href="https://openxbl.com" target="_blank" rel="noopener noreferrer" style={{ color:C.green, textDecoration:"none", fontWeight:600 }}>openxbl.com ↗</a><br/>
              2. Click <b style={{ color:C.text }}>Sign In</b> with your Xbox / Microsoft account<br/>
              3. Copy your <b style={{ color:C.text }}>API Key</b> from the dashboard
            </div>
          </div>

          <div style={{ fontSize:10, color:"#444", letterSpacing:"2px", textTransform:"uppercase", marginBottom:8, fontWeight:600 }}>Paste API Key</div>
          <input value={key} onChange={e => { setKey(e.target.value); setPreview(null); setError(""); }}
            placeholder="Paste key here…"
            style={{ width:"100%", background:C.faint, border:`0.5px solid ${error ? C.pink : C.border}`, borderRadius:8, padding:"13px 14px", color:C.text, fontSize:13, outline:"none", boxSizing:"border-box", fontFamily:"monospace", marginBottom:10 }} />
          ...
          {preview ? (
            <>
              <div style={{ display:"flex", alignItems:"center", gap:14, padding:"14px 16px", background:C.faint, borderRadius:10, marginBottom:16, border:`0.5px solid ${C.green}44` }}>
                ...
                <div>
                  <div style={{ fontSize:14, fontWeight:500 }}>{preview.gamertag}</div>
                  <div style={{ fontSize:11, color:C.green }}>Xbox account verified ✓</div>
                </div>
              </div>
              <button onClick={() => onConnect(key.trim(), preview)} style={{ width:"100%", padding:14, borderRadius:10, border:"none", background:C.green, color:"#fff", fontWeight:600, fontSize:14, cursor:"pointer", letterSpacing:"1px" }}>
                Connect Xbox
              </button>
            </>
          ) : (
            <button onClick={verify} disabled={loading || !key.trim()} style={{ width:"100%", padding:14, borderRadius:10, border:"none", background:key.trim() ? C.green : C.faint, color:key.trim() ? "#fff" : C.muted, fontWeight:600, fontSize:14, cursor:key.trim()?"pointer":"default", letterSpacing:"1px", transition:"all .15s" }}>
...
function HomeScreen({ games, logs, onGameClick, steamId, onConnectSteam, psnToken, onConnectPSN, xboxKey, onConnectXbox, isDesktop, user, onOpenWrapped }) {
  ...
  const PLATFORM_CONNECT = [
    ...
    !psnToken && { label:"PlayStation",  desc:"Trophies & PS4/PS5 library",      color:"#0070d1", bg:"rgba(0,36,80,.7)",      border:"rgba(0,120,255,.2)", onClick:onConnectPSN,
...
function DiaryScreen({ logs, onGameClick }) {
  ...
  return (
    <div style={{ paddingBottom:90, color:C.text }}>
      <div style={{ padding:"clamp(52px,12vh,72px) clamp(18px,5vw,48px) clamp(14px,3vh,20px)", position:"sticky", top:0, background:"rgba(6,8,16,0.96)", backdropFilter:"blur(24px)", zIndex:10, borderBottom:`0.5px solid rgba(34,85,204,0.15)` }}>
        <div style={{ fontFamily:"monospace", fontSize:9, color:C.blue, letterSpacing:"3px", marginBottom:6, opacity:0.65 }}>&gt;_ OPERATIVE_LOG://</div>
        <div style={{ fontSize:"clamp(20px,4vw,28px)", fontWeight:700, letterSpacing:"-0.3px", marginBottom:"clamp(14px,2vh,20px)", fontFamily:"monospace" }}>DIARY</div>
  ...
  return (
    <div style={{ paddingBottom:90, color:C.text }}>
      <div style={{ padding:"clamp(52px,12vh,72px) clamp(18px,5vw,48px) clamp(14px,3vh,20px)", position:"sticky", top:0, background:"rgba(6,8,16,0.96)", backdropFilter:"blur(24px)", zIndex:10, borderBottom:`0.5px solid rgba(34,85,204,0.15)` }}>
        {filterMode ? (
          <div>
            <button onClick={clearFilter} style={{ background:"none", border:"none", color:C.blue, fontSize:9, cursor:"pointer", padding:0, fontFamily:"monospace", letterSpacing:"2px", marginBottom:"clamp(10px,2vh,16px)" }}>&lt; BACK://BROWSE</button>
            <div style={{ fontSize:"clamp(20px,4vw,28px)", fontWeight:700, letterSpacing:"-0.3px", fontFamily:"monospace" }}>
              {(filterMode.type === "genre" ? filterMode.name : `BY_${filterMode.name.toUpperCase().replace(/ /g,"_")}`)}
            </div>
          </div>
        ) : (
          <div>
            <div style={{ fontFamily:"monospace", fontSize:9, color:C.blue, letterSpacing:"3px", marginBottom:8, opacity:0.65 }}>&gt;_ SEARCH_DB://IGDB</div>
            <div style={{ position:"relative", marginBottom:"clamp(12px,2vh,18px)" }}>
              <span style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)", fontFamily:"monospace", fontSize:12, color:`${C.blue}80`, pointerEvents:"none" }}>&gt;</span>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="query_games…"
                style={{ width:"100%", background:"rgba(8,16,36,0.7)", border:`0.5px solid rgba(34,85,204,0.3)`, borderRadius:2, padding:"clamp(10px,2vh,13px) 14px clamp(10px,2vh,13px) 30px", color:C.text, fontSize:13, fontFamily:"monospace", outline:"none", boxSizing:"border-box", letterSpacing:"0.5px" }} />
            </div>
            {searchingOnline && (
  ...
  if (open) {
    ...
    return (
      <div style={{ minHeight:"100vh", background:C.bg, color:C.text, paddingBottom:90 }}>
        <div style={{ padding:"clamp(52px,12vh,72px) clamp(18px,5vw,48px) clamp(16px,3vh,24px)", background:"rgba(6,8,16,0.96)", backdropFilter:"blur(24px)", position:"sticky", top:0, zIndex:10, borderBottom:`0.5px solid rgba(34,85,204,0.15)` }}>
          <button onClick={()=>{setOpen(null);setEditing(false);setAdding(false);}} style={{ background:"none", border:"none", color:C.blue, fontSize:9, cursor:"pointer", marginBottom:"clamp(12px,2vh,18px)", padding:0, fontFamily:"monospace", letterSpacing:"2px" }}>&lt; BACK://LISTS</button>
          {editing ? (
            <>
  ...
  return (
    <div style={{ paddingBottom:90, color:C.text }}>
      <div style={{ padding:"clamp(52px,12vh,72px) clamp(18px,5vw,48px) clamp(20px,4vh,28px)" }}>
        <div style={{ fontFamily:"monospace", fontSize:9, color:C.blue, letterSpacing:"3px", marginBottom:6, opacity:0.65 }}>&gt;_ USER_LISTS://</div>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
          <div style={{ fontSize:"clamp(20px,4vw,28px)", fontWeight:700, letterSpacing:"-0.3px", fontFamily:"monospace" }}>LISTS</div>
          <button onClick={()=>setAdding(true)} style={{ background:"transparent", border:`0.5px solid ${C.pink}`, color:C.pink, padding:"7px 16px", borderRadius:2, fontSize:9, fontWeight:600, cursor:"pointer", letterSpacing:"1.5px", fontFamily:"monospace" }}>+ NEW</button>
        </div>
        {adding && (
...
// ── PROFILE ───────────────────────────────────────────────────────────────────
function ProfileScreen({ user, logs, displayName, photoUrl, onGameClick }) {
...
function PlatformsScreen({
  ...
  return (
    <div style={{ paddingBottom:90, color:C.text }}>
      <div style={{ padding:"clamp(52px,12vh,72px) clamp(18px,5vw,48px) clamp(20px,4vh,28px)", position:"sticky", top:0, background:"rgba(6,8,16,0.96)", backdropFilter:"blur(24px)", zIndex:10, borderBottom:`0.5px solid rgba(34,85,204,0.15)` }}>
        <div style={{ fontFamily:"monospace", fontSize:9, color:C.blue, letterSpacing:"3px", marginBottom:6, opacity:0.65 }}>&gt;_ PLATFORM_LINK://</div>
        <div style={{ fontSize:"clamp(20px,4vw,28px)", fontWeight:700, letterSpacing:"-0.3px", marginBottom:4, fontFamily:"monospace" }}>PLATFORMS</div>
        <div style={{ fontFamily:"monospace", fontSize:10, color:"#3a4a6a", letterSpacing:"0.5px" }}>// connect accounts to sync your library</div>
      </div>
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent;font-family:'Poppins',sans-serif}
        ...
        html,body{
          ...
          background-image:
            linear-gradient(rgba(34,85,204,0.055) 1px,transparent 1px),
            linear-gradient(90deg,rgba(34,85,204,0.055) 1px,transparent 1px),
            repeating-linear-gradient(0deg,transparent,transparent 3px,rgba(0,0,0,0.07) 3px,rgba(0,0,0,0.07) 4px);
          background-size:44px 44px,44px 44px,4px 4px;
      ...
      <div style={{ width:"100%" }}>
        ...
        {!detail && (
          <>
            ...
            <div style={{ position:"fixed", top:0, left:0, right:0, zIndex:200, padding:"clamp(12px,2.5vw,18px) clamp(18px,5vw,48px)", display:"flex", alignItems:"center", justifyContent:"space-between", background:"rgba(6,8,16,0.88)", backdropFilter:"blur(24px)", borderBottom:`0.5px solid rgba(34,85,204,0.18)` }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:9, pointerEvents:"none" }}>
                ...
                <span style={{ fontFamily:"monospace", fontSize:"clamp(14px,2vw,17px)", fontWeight:600, letterSpacing:"0.12em", color:C.text }}>ortana</span>
              </div>
        ...
        {detail ? (
          ...
          <div style={{ display: isDesktop && tab==="home" ? "flex" : "block", alignItems:"flex-start", width:"100%" }}>
            ...
            <div style={{ flex:1, minWidth:0, overflow:"hidden" }}>
              ...
              />}
              {tab==="profile" && <ProfileScreen user={user} logs={logs} displayName={displayName} photoUrl={photoUrl} onGameClick={setDetail} />}
            </div>
        ...
        {/* ── Bottom tab bar ── */}
        {!detail && (
          <div style={{ position:"fixed", bottom:0, left:0, right:0, background:"rgba(6,8,16,0.97)", backdropFilter:"blur(24px)", borderTop:`0.5px solid rgba(34,85,204,0.18)`, display:"grid", gridTemplateColumns:`repeat(${TABS.length},1fr)`, zIndex:100, paddingBottom:"clamp(10px,env(safe-area-inset-bottom,0px)+10px,28px)", overflow:"hidden" }}>
            <div style={{ position:"absolute", top:0, left:0, right:0, height:"1px", background:`linear-gradient(90deg,transparent,${C.blue}80,${C.pink}70,${C.yellow}60,${C.green}70,transparent)` }} />
            {TABS.map(t=>(
              <button key={t.key} onClick={()=>setTab(t.key)} style={{ background:"none", border:"none", cursor:"pointer", padding:"13px 4px 3px", display:"flex", flexDirection:"column", alignItems:"center", gap:5 }}>
                <div style={{ width:16, height:2, borderRadius:1, background:tab===t.key?C.blue:"transparent", boxShadow:tab===t.key?`0 0 8px ${C.blue}`:undefined, transition:"all .2s" }} />
                <span style={{ fontFamily:"monospace", fontSize:9, fontWeight:tab===t.key?700:400, letterSpacing:"1.5px", color:tab===t.key?C.blue:"#3a4a6a", transition:"color .15s" }}>{t.key.toUpperCase()}</span>
              </button>
            ))}
          </div>
        )}
